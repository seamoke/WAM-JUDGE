...
# LeRobot v2.1 parquet files use legacy "_type": "List"; map to Sequence for datasets>=2.19.
_orig_generate_from_dict = _datasets_features.generate_from_dict

def _generate_from_dict_with_list(obj):
    if isinstance(obj, dict) and obj.get("_type") == "List":
        obj = dict(obj)
        obj["_type"] = "Sequence"
    return _orig_generate_from_dict(obj)

_datasets_features.generate_from_dict = _generate_from_dict_with_list


def pad_latent_batch_collate(batch):
    """Pad variable-length (F) latents/actions so batch_size > 1 can be stacked."""
    max_f = max(item['latents'].shape[1] for item in batch)

    latents, actions, actions_mask, text_embs, latents_mask = [], [], [], [], []
    for item in batch:
        f = item['latents'].shape[1]
        pad_f = max_f - f

        lat = item['latents'].contiguous().clone()
        act = item['actions'].contiguous().clone()
        act_mask = item['actions_mask'].contiguous().clone()
        if pad_f > 0:
            # Tensors are (C, F, H, W) or (C, F, N, 1); pad along F.
            lat = torch.nn.functional.pad(lat, (0, 0, 0, 0, 0, pad_f))
            act = torch.nn.functional.pad(act, (0, 0, 0, 0, 0, pad_f))
            act_mask = torch.nn.functional.pad(act_mask, (0, 0, 0, 0, 0, pad_f))

        mask = torch.zeros(max_f, dtype=torch.bool)
        mask[:f] = True

        latents.append(lat)
        actions.append(act)
        actions_mask.append(act_mask)
        text_embs.append(item['text_emb'].contiguous().clone())
        latents_mask.append(mask)
    ...
    return {
        ...
        'latents_mask': torch.stack(latents_mask, dim=0),
    }
...
def construct_lerobot(
    ...
    try:
        ...
        print(f"[LatentLeRobotDataset] skip {repo_id}: {e}")
        return None
    ...
    def __init__(
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Missing local LeRobot parquet data under {self.root}"
            ) from e
