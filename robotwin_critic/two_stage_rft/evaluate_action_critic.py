"""Evaluate Action Critic on held-out Stage-2 real and corrupted segments."""
...
def evaluate(
    ...
    *,
    ...
    profile = ActionCriticProfile.from_json(profile_path)
    ...
    for ref in refs:
        ...
        for start, end in episode_segments(ref.repo, ref.output_episode_index):
            ...
            if len(segment) < 8:
                continue
            result = score_actions(segment, profile)
            real_scores.append(result["action_score"])
            real_accepts.append(result["accepted"])
            for kind in kinds:
                corrupted = score_actions(corrupt(segment, kind), profile)
    ...
    return {
        "evaluation_scope": "stage2_actions_for_evaluation_only",
