...
EXPECTED_GLOBAL_BATCH=$((TRAIN_BATCH_SIZE_PER_GPU * NGPU * GRADIENT_ACCUMULATION_STEPS))
if (( TRAIN_GLOBAL_BATCH != EXPECTED_GLOBAL_BATCH )); then
  echo "TRAIN_GLOBAL_BATCH must equal TRAIN_BATCH_SIZE_PER_GPU*number_of_GPUs*GRADIENT_ACCUMULATION_STEPS ($EXPECTED_GLOBAL_BATCH)" >&2
  exit 2
fi
