...
class VA_Server:
    ...
    def __init__(self, job_config):
        ...
        val = os.environ.get("WAN_VA_SAVE_INFER_DEBUG")
        if val is not None:
            self.save_infer_debug = val.strip().lower() in ("1", "true", "yes")
        else:
            self.save_infer_debug = bool(getattr(job_config, "save_infer_debug", False))
        ...
        offload_env = os.environ.get("WAN_VA_ENABLE_OFFLOAD")
        if offload_env is None:
            offload_default = getattr(job_config, "enable_offload", True)
        else:
            offload_default = offload_env.strip().lower() in (
                "1", "true", "yes"
            )
        self.offload_vae = env_bool("WAN_VA_OFFLOAD_VAE", offload_default)
        self.offload_text_encoder = env_bool(
            "WAN_VA_OFFLOAD_TEXT_ENCODER", offload_default
        )
        self.swap_text_encoder_for_prompt = env_bool(
            "WAN_VA_SWAP_TEXT_ENCODER_FOR_PROMPT", False
        )
        self.prompt_embed_cache_dir = os.environ.get(
            "WAN_VA_PROMPT_EMBED_CACHE_DIR", ""
        ).strip()
        self.prompt_embed_cache_strict = env_bool(
            "WAN_VA_PROMPT_EMBED_CACHE_STRICT", False
        )
        self.prompt_embed_service = os.environ.get(
            "WAN_VA_PROMPT_EMBED_SERVICE", ""
        ).strip()
        self._cached_negative_prompt_embed = None
        self.enable_offload = self.offload_vae
        ...
        self.vae = load_vae(
            ...
            torch_device='cpu' if self.offload_vae else self.device,
        )
        ...
        if self.prompt_embed_cache_dir and self.prompt_embed_cache_strict:
            self.text_encoder = None
            logger.info(
                "Using strict prompt embedding cache: %s",
                self.prompt_embed_cache_dir,
            )
        else:
        ...
        swap_edge_blocks = int(os.environ.get("WAN_VA_SWAP_EDGE_BLOCKS", "0"))
        if swap_edge_blocks:
            self._enable_edge_block_swap(swap_edge_blocks)
    ...
    def _enable_edge_block_swap(self, count):
        blocks = self.transformer.blocks
        if count < 1 or count * 2 >= len(blocks):
            raise ValueError(
                f"WAN_VA_SWAP_EDGE_BLOCKS must be in [1, {len(blocks) // 2 - 1}]"
            )

        front = list(blocks[:count])
        back = list(blocks[-count:])

        def move_group_to_cpu(group):
            for block in group:
                block.to("cpu")
            torch.cuda.empty_cache()

        def move_group_to_device(group):
            for block in group:
                block.to(self.device)

        def group_is_on_cpu(group):
            return next(group[0].parameters()).device.type == "cpu"

        def activate_front(_module, _args):
            if group_is_on_cpu(front):
                move_group_to_cpu(back)
                move_group_to_device(front)

        def activate_back(_module, _args):
            if group_is_on_cpu(back):
                move_group_to_cpu(front)
                move_group_to_device(back)

        move_group_to_cpu(front)
        blocks[0].register_forward_pre_hook(activate_front)
        blocks[-count].register_forward_pre_hook(activate_back)
        logger.info(
            "Enabled exact edge-block swapping for %d front/back blocks", count
        )
    ...
    def encode_prompt_with_runtime_swap(self, **kwargs):
        cached = self._load_cached_prompt_embeds(
            kwargs.get("prompt"),
            kwargs.get("do_classifier_free_guidance", True),
            kwargs.get("device") or self.device,
            kwargs.get("dtype") or self.dtype,
        )
        if cached is not None:
            return cached

        if not self.swap_text_encoder_for_prompt:
            return self.encode_prompt(**kwargs)

        text_device = next(self.text_encoder.parameters()).device
        if text_device.type != "cpu":
            return self.encode_prompt(**kwargs)
        ...
        try:
            ...
            prompt_embeds = self.encode_prompt(**kwargs)
            torch.cuda.synchronize(self.device)
            return prompt_embeds
        finally:
            self.text_encoder.to(text_device)
            torch.cuda.empty_cache()
            if vae_device.type == "cuda":
                self.vae.to(vae_device)
            torch.cuda.empty_cache()
            logger.info(
                "Prompt encoder runtime swap completed in %.2fs",
                time.perf_counter() - started_at,
            )

    def _load_cached_prompt_embeds(self, prompt, use_negative, device, dtype):
        if not self.prompt_embed_cache_dir:
            return None
        if not isinstance(prompt, str):
            if self.prompt_embed_cache_strict:
                raise TypeError(
                    f"Strict prompt cache requires a string prompt, got {type(prompt)}"
                )
            return None

        key = hashlib.sha256(prompt.encode("utf-8")).hexdigest()
        positive_path = os.path.join(self.prompt_embed_cache_dir, f"{key}.pt")
        negative_path = os.path.join(self.prompt_embed_cache_dir, "negative.pt")
        if not os.path.isfile(positive_path) and self.prompt_embed_service:
            self._request_prompt_embedding(prompt, key)
        if not os.path.isfile(positive_path) or (
            use_negative and not os.path.isfile(negative_path)
        ):
            message = f"Prompt embedding cache miss for key={key} prompt={prompt!r}"
            if self.prompt_embed_cache_strict:
        ...
        negative = None
        if use_negative:
            if self._cached_negative_prompt_embed is None:
                self._cached_negative_prompt_embed = torch.load(
                    negative_path, map_location="cpu", weights_only=True
                )
            negative = self._cached_negative_prompt_embed.to(
                device=device, dtype=dtype
            )
        logger.info("Loaded prompt embedding cache key=%s", key)
        return positive, negative

    def _request_prompt_embedding(self, prompt, expected_key):
        host, port_text = self.prompt_embed_service.rsplit(":", 1)
        timeout = float(os.environ.get("WAN_VA_PROMPT_EMBED_SERVICE_TIMEOUT", "300"))
        request = json.dumps(
            {"op": "encode", "prompt": prompt}, ensure_ascii=False
        ) + "\n"
        ...
        if not response_line:
            raise RuntimeError("Prompt embedding service closed without a response")
        response = json.loads(response_line.decode("utf-8"))
        if not response.get("ok"):
            raise RuntimeError(
                f"Prompt embedding service failed: {response.get('error')}"
            )
        if response.get("key") != expected_key:
            raise RuntimeError(
                f"Prompt embedding key mismatch: {response.get('key')} != {expected_key}"
            )
    ...
    def _reset(self, prompt=None):
        ...
        if prompt is None:
            ...
            self.prompt_embeds, self.negative_prompt_embeds = self.encode_prompt_with_runtime_swap(
                prompt=prompt,
                negative_prompt=None,
...
def run(args):    
    ...
    if getattr(args, "save_infer_debug", False):
        config.save_infer_debug = True
...
def main():
    ...
    parser.add_argument(
        "--save-infer-debug",
        action="store_true",
        help="Save obs/latents/actions .pt under save_root/real/ during infer",
    )
