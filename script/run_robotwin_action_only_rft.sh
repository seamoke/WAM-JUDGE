...
if (( TARGET_GLOBAL_BATCH % denominator != 0 )); then
  echo "TARGET_GLOBAL_BATCH must be divisible by BATCH_SIZE*NGPU." >&2
  exit 2
fi
...

{
  ...
  echo "real_source_update_ratio=0.7"
  echo "pseudo_source_update_ratio=0.3"
  echo "objective=action_flow_matching_only"
  echo "trainable_modules=action_embedder,condition_embedder_action.time_embedder,condition_embedder_action.time_proj,action_proj_out"
...
if (( rc == 0 )); then
  set +e
  "$WAM_PYTHON" -m robotwin_critic.two_stage_rft.audit_action_only_checkpoint \
