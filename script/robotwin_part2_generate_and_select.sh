...
"$WAM_PYTHON" -m robotwin_critic.two_stage_rft.decode_wam_candidates \
...
"$VLAC_PYTHON" -m robotwin_critic.two_stage_rft.score_vlac_candidates \
...
for mode in naive process action dual; do
  "$WAM_PYTHON" -m robotwin_critic.two_stage_rft.select_rft_candidates \
