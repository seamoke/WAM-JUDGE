...
_lingbot_root = Path(__file__).resolve().parents[2]
if str(_lingbot_root) not in sys.path:
    sys.path.insert(0, str(_lingbot_root))
from evaluation.robotwin.vulkan_env import configure_robotwin_vulkan, vulkan_gpu_sim_slot

configure_robotwin_vulkan()

robowin_root = Path(
    os.environ.get("ROBOTWIN_DIR", "/workspace/lingbot-va/third_party/RoboTwin")
).resolve()
if not robowin_root.is_dir():
    raise SystemExit(
        f"ROBOTWIN_DIR not found: {robowin_root}\n"
        "Run: bash script/setup_robotwin_eval.sh"
    )
...
from evaluation.robotwin.eval_timing import EvalTiming, timing_enabled
import time
...
def _load_seed_cache_for_task(cache_path: str | None, task_name: str, task_config: str) -> list[dict]:
    if not cache_path:
        return []
    path = Path(cache_path)
    if not path.is_file():
        raise FileNotFoundError(f"ROBOTWIN_SEED_CACHE not found: {path}")
    with open(path, "r", encoding="utf-8") as f:
        payload = json.load(f)
    if payload.get("task_config") not in (None, task_config):
        print(
            f"WARNING: seed cache task_config={payload.get('task_config')} "
            f"does not match requested task_config={task_config}"
        )
    tasks = payload.get("tasks", {})
    entries = tasks.get(task_name, [])
    if not isinstance(entries, list):
        raise ValueError(f"Invalid seed cache entries for task {task_name}: expected list")
    print(f"Loaded {len(entries)} cached eval seeds for task={task_name} from {path}")
    return entries


def _cached_episode_info(entry: dict) -> dict | None:
    info = entry.get("episode_info")
    return info if isinstance(info, dict) else None


def _choose_instruction(results, instruction_type: str, seed: int):
    choices = results[0][instruction_type]
    rng = np.random.default_rng(seed)
    return rng.choice(choices)


def _load_partial_progress(save_root, st_seed, task_name, test_num):
    if not _env_flag("ROBOTWIN_RESUME_PARTIAL", False):
        return 0, 0, []

    seed_root = Path(save_root) / f"stseed-{st_seed}"
    result_file = seed_root / "metrics" / task_name / "res.json"
    if not result_file.is_file():
        return 0, 0, []

    with open(result_file, "r", encoding="utf-8") as f:
        result = json.load(f)
    completed = int(result.get("total_num", 0))
    successes = int(result.get("succ_num", 0))
    if not (0 <= successes <= completed <= test_num):
        raise RuntimeError(f"Invalid partial result in {result_file}: {result}")

    timing_file = seed_root / "eval_timing" / f"{task_name}.jsonl"
    completed_seeds = []
    if timing_enabled():
        if not timing_file.is_file():
            raise RuntimeError(
                f"Partial result has {completed} episodes but timing file is missing: {timing_file}"
            )
        with open(timing_file, "r", encoding="utf-8") as f:
            records = [json.loads(line) for line in f if line.strip()]
        episodes = [int(record.get("episode", -1)) for record in records]
        if episodes != list(range(1, completed + 1)):
            raise RuntimeError(
                f"Partial timing episodes do not match res.json for {task_name}: {episodes}"
            )
        try:
            completed_seeds = [int(record["seed"]) for record in records]
        except (KeyError, TypeError, ValueError) as e:
            raise RuntimeError(
                f"Partial timing records contain invalid seeds for {task_name}: {timing_file}"
            ) from e

    print(
        f"Resuming task={task_name} from {completed}/{test_num} episodes "
        f"and {successes} successes",
        flush=True,
    )
    return completed, successes, completed_seeds


def _seed_cache_resume_index(seed_cache_entries, completed_seeds, task_name):
    cache_seeds = [int(entry["seed"]) for entry in seed_cache_entries]
    if len(cache_seeds) != len(set(cache_seeds)):
        raise RuntimeError(f"Seed cache contains duplicate seeds for task={task_name}")

    next_index = 0
    for episode, seed in enumerate(completed_seeds, start=1):
        while next_index < len(cache_seeds) and cache_seeds[next_index] != seed:
            next_index += 1
        if next_index >= len(cache_seeds):
            raise RuntimeError(
                f"Completed seed sequence is not an ordered subsequence of the cache "
                f"for task={task_name}: episode={episode} seed={seed}"
            )
        next_index += 1
    return next_index
...
def main(usr_args):
    ...
    if os.environ.get("ROBOTWIN_EVAL_VIDEO_LOG") is not None:
        args["eval_video_log"] = _env_flag("ROBOTWIN_EVAL_VIDEO_LOG", False)
    ...
    prompt_capture_dir = os.environ.get("ROBOTWIN_CAPTURE_PROMPT_DIR", "").strip()
    model = None if prompt_capture_dir else WebsocketClientPolicy(port=usr_args['port'])

    expert_check = usr_args.get(
        "expert_check",
        _env_flag("ROBOTWIN_EXPERT_CHECK", True),
    )
    seed_cache_entries = _load_seed_cache_for_task(
        usr_args.get("seed_cache") or os.environ.get("ROBOTWIN_SEED_CACHE"),
        task_name,
        task_config,
    )
    save_visualization = usr_args.get(
        "save_visualization",
        _env_flag("ROBOTWIN_SAVE_VISUALIZATION", False),
    )
    save_comparison_video = usr_args.get(
        "save_comparison_video",
        _env_flag("ROBOTWIN_SAVE_COMPARISON_VIDEO", False),
    )
    ...
    st_seed, suc_num = eval_policy(task_name,
                                   ...
                                   write_comparison_video=save_comparison_video,
...
def eval_policy(task_name,
                ...
                write_comparison_video=False,
                video_guidance_scale=5.0,
                action_guidance_scale=5.0,
                seed_cache_entries=None):
    print(f"\033[34mTask Name: {args['task_name']}\033[0m")
    print(f"\033[34mPolicy Name: {args['policy_name']}\033[0m")

    completed, successes, completed_seeds = _load_partial_progress(
        args["save_root"], st_seed, task_name, test_num
    )
    ...
    succ_seed = completed
    suc_test_seed_list = []
    seed_cache_entries = seed_cache_entries or []
    seed_cache_idx = completed
    if seed_cache_entries and completed:
        if not completed_seeds:
            raise RuntimeError(
                f"Cannot safely resume cached seeds for task={task_name} without timing records"
            )
        seed_cache_idx = _seed_cache_resume_index(
            seed_cache_entries, completed_seeds, task_name
        )
        print(
            f"Resuming seed cache for task={task_name} at index={seed_cache_idx} "
            f"after seed={completed_seeds[-1]}",
            flush=True,
        )


    now_seed = st_seed + completed
    clear_cache_freq = args["clear_cache_freq"]

    episode_chunk = int(os.environ.get("ROBOTWIN_CLIENT_EPISODE_CHUNK", "0"))
    stop_after = test_num
    if episode_chunk > 0:
        stop_after = min(test_num, completed + episode_chunk)

    args["eval_mode"] = True
    eval_timing = (
        EvalTiming(args["task_name"], args["save_root"], st_seed)
        if timing_enabled()
        else None
    )
    trace_eval = os.environ.get("ROBOTWIN_SETUP_TRACE", "0") == "1"

    def trace(stage):
        if trace_eval:
            print(f"[eval_trace] task={task_name} stage={stage}", flush=True)

    while succ_seed < stop_after:
        cache_entry = None
        cached_info = None
        if seed_cache_entries:
            if seed_cache_idx >= len(seed_cache_entries):
                raise RuntimeError(
                    f"Seed cache for {task_name} has only {len(seed_cache_entries)} entries; "
                    f"need {test_num} usable episodes"
                )
            cache_entry = seed_cache_entries[seed_cache_idx]
            seed_cache_idx += 1
            now_seed = int(cache_entry["seed"])
            cached_info = _cached_episode_info(cache_entry)
        ...
        if cached_info is not None:
            ...
            TASK_ENV.fill_episode_info_for_eval()
            episode_params = TASK_ENV.info.get("info") or {}
            episode_info_list = [episode_params]

        results = generate_episode_descriptions(args["task_name"], episode_info_list, test_num)
        instruction = _choose_instruction(results, instruction_type, now_seed)
        TASK_ENV.set_instruction(instruction=instruction)  # set language instruction

        prompt_capture_dir = os.environ.get("ROBOTWIN_CAPTURE_PROMPT_DIR", "").strip()
        if prompt_capture_dir:
            prompt = str(TASK_ENV.get_instruction())
            capture_path = Path(prompt_capture_dir) / f"{task_name}.jsonl"
            ...
            now_id += 1
            TASK_ENV.close_env(clear_cache=(succ_seed % clear_cache_freq == 0))
            if TASK_ENV.render_freq:
                TASK_ENV.viewer.close()
            TASK_ENV.test_num += 1
            print(
                f"Captured prompt {task_name} episode={succ_seed} seed={now_seed}",
                flush=True,
            )
            continue
        ...
        trace("get_instruction:begin")
        prompt = TASK_ENV.get_instruction()
        trace("get_instruction:end")
        trace("infer_reset:begin")
        ...
        trace("initial_obs:begin")
        with _timing_section(eval_timing, "obs"):
            initial_obs = TASK_ENV.get_obs() 
        trace("initial_obs:end")
        ...
        while TASK_ENV.take_action_cnt<TASK_ENV.step_lim:
            if first:
                trace("first_obs:begin")
                with _timing_section(eval_timing, "obs"):
                    observation = TASK_ENV.get_obs()
                    first_obs = format_obs(observation, prompt)
                trace("first_obs:end")

            trace("infer_action:begin")
        ...
        if not seed_cache_entries:
            now_seed += 1

    if eval_timing:
        eval_timing.finish_run(episodes=TASK_ENV.test_num, successes=TASK_ENV.suc)
