...
from dataset import MultiLatentLeRobotDataset, pad_latent_batch_collate
import gc


def _swanlab_config(config):
    out = {}
...
class Trainer:
    def __init__(self, config):
        ...
        if getattr(config, "enable_swanlab", False) and config.rank == 0:
            ...
            swanlab_mode = os.getenv("LINGBOT_SWANLAB_MODE", "online").strip().lower()
            if swanlab_mode not in {"online", "local", "offline"}:
                raise ValueError(f"Unsupported LINGBOT_SWANLAB_MODE={swanlab_mode!r}")
            if swanlab_mode == "online":
                api_key = os.getenv("SWANLAB_API_KEY")
                if not api_key:
                    raise RuntimeError(
                        "LINGBOT_SWANLAB_MODE=online but SWANLAB_API_KEY is not set"
                    )
                swanlab.login(api_key=api_key, save=False)

            self.swanlab_run = swanlab.init(
                project=os.getenv("LINGBOT_SWANLAB_PROJECT", "lingbot-va-libero"),
                workspace=os.getenv("LINGBOT_SWANLAB_WORKSPACE"),
                name=os.getenv("LINGBOT_SWANLAB_EXPERIMENT_NAME", config.__name__),
                config=_swanlab_config(config),
                mode=swanlab_mode,
                log_dir=os.getenv("LINGBOT_SWANLAB_LOG_DIR"),
            )
            self._swanlab = swanlab
            logger.info(f"SwanLab logging enabled (mode={swanlab_mode})")
            try:
                logger.info(f"SwanLab dashboard: {self.swanlab_run.url}")
            except ValueError:
                pass
        self.step = 0
        self.config = config
        ...
        if getattr(config, 'enable_activation_checkpointing', True):
            apply_ac(self.transformer)
        else:
            logger.info("Activation checkpointing disabled (faster, more VRAM)")
        ...
        lr_scheduler_type = getattr(config, "lr_scheduler_type", "constant").strip().lower()
        ...
        logger.info(
            "LR scheduler=%s, warmup_steps=%d, total_steps=%d, min_lr_ratio=%.4f",
            lr_scheduler_type,
            config.warmup_steps,
            config.num_steps,
            getattr(config, "min_lr_ratio", 0.0),
        )
        ...
        train_dataset = MultiLatentLeRobotDataset(
            config=config,
            num_init_worker=getattr(config, 'dataset_init_workers', 128),
        )
        ...
        collate_fn = pad_latent_batch_collate if config.batch_size > 1 else None
        loader_kwargs = dict(
    ...
    def _prepare_input_dict(self, batch_dict):
        ...
        if 'latents_mask' in batch_dict:
            frame_mask = batch_dict['latents_mask'][:, None, :, None, None].float()
            for key in ('noisy_latents', 'targets', 'latent'):
                latent_dict[key] = latent_dict[key] * frame_mask
            latent_dict['latents_mask'] = batch_dict['latents_mask']

        input_dict = {
            'latent_dict': latent_dict,
            'action_dict': action_dict,
            # Fixed attention pattern avoids flex-attn mask recompile every step.
            'chunk_size': getattr(self.config, 'frame_chunk_size', 4),
            'window_size': getattr(self.config, 'attn_window', 30),
        }
        return input_dict
    ...
    def compute_loss(self,
        ...
        per_frame_latent_loss = latent_loss_per_frame / (latent_mask_per_frame + 1e-6)
        if 'latents_mask' in input_dict['latent_dict']:
            valid_frame = input_dict['latent_dict']['latents_mask'].flatten().float()
            latent_loss = (per_frame_latent_loss * valid_frame).sum() / valid_frame.sum().clamp(min=1)
        else:
            latent_loss = per_frame_latent_loss.mean()
        ...
        per_frame_action_loss = action_loss_per_frame / (action_mask_per_frame + 1e-6)
        if 'latents_mask' in input_dict['latent_dict']:
            valid_frame = input_dict['latent_dict']['latents_mask'].flatten().float()
            action_loss = (per_frame_action_loss * valid_frame).sum() / valid_frame.sum().clamp(min=1)
        else:
            action_loss = per_frame_action_loss.mean()
    ...
    def _finish_swanlab(self):
        if (
            getattr(self.config, "enable_swanlab", False)
        ...
        try:
            while self.step < self.config.num_steps:
                ...
                if losses['should_log']:
                    ...
                    gc_interval = getattr(self.config, 'gc_interval', 0)
                    ...
                    if self.config.rank == 0:
                        ...
                        if getattr(self.config, "enable_swanlab", False):
                            self._swanlab.log({
                    ...
                    save_steps = set(getattr(self.config, "save_steps", []))
                    should_save = (
                        self.step in save_steps
                        if save_steps
                        else self.step % self.config.save_interval == 0
                    )
                    if should_save:
                        if self.config.rank == 0:
                ...
                if losses['should_log'] and dist.is_initialized():
                    dist.barrier()
        finally:
            progress_bar.close()
